...
### Line splitting is similar to NyuFX: there are `word`, `syll` and `char` objects.
...
The vanilla templater blanks out `line.text` and uses it as a buffer to generate the output line. This means the value for `line.text` generated by karaskel was inaccessible at least in some versions of this modified templater, and possibly the vanilla templater. I recently fixed that.
